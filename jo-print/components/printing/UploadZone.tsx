...
export default function UploadZone({ onFileSelect }: UploadZoneProps) {
  ...
  const processFile = (file: File) => {
    if (file.size > MAX_SIZE) {
      setSizeError(`حجم الملف (${(file.size / 1024 / 1024).toFixed(1)} MB) يتجاوز الحد الأقصى 50 MB`)
      return
    }
    setSizeError('')
    ...
    <div>
      {!uploadedFile ? (
        <>
          ...
          {sizeError && (
            <p className="mt-2 text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2">{sizeError}</p>
          )}
        </>
      ) : (
        <div className="border border-green-200 bg-green-50 rounded-xl p-4 flex items-center gap-3">
          ...
          <button onClick={() => { setUploadedFile(null); setSizeError('') }} className="text-gray-400 hover:text-gray-600">
            <X size={18} />
          </button>
        </div>
      )}
